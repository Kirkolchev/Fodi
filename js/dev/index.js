...
function initSliders() {
  if (document.querySelector(".swiper")) {
    let colorizeSlides2 = function() {
      const slides = document.querySelectorAll(".coments__slide");
      const colors = ["#eb9d20", "#ef7c6a", "#f855ca", "#9C27B0", "#009688", "#FFC107"];
      slides.forEach((slide2, index) => {
        const color = colors[index % colors.length];
        const svgDecor = slide2.querySelectorAll(".item-coments__decor svg use");
        svgDecor.forEach((use) => {
          const svg = use.closest("svg");
          svg.style.fill = color;
        });
        const ratingBlock = slide2.querySelector("[data-fls-rating]");
        if (!ratingBlock) return;
        const ratingValue = +ratingBlock.dataset.flsRatingValue || 0;
        const ratingItems = ratingBlock.querySelectorAll(".rating__item");
        const full = Math.floor(ratingValue);
        const partial = ratingValue - full;
        ratingItems.forEach((item, i) => {
          var _a;
          const starIndex = i + 1;
          item.classList.remove("rating__item--active");
          (_a = item.querySelector("span")) == null ? void 0 : _a.remove();
          item.style.backgroundImage = `url("data:image/svg+xml;base64,${btoa(generateStar2(false, color))}")`;
          if (starIndex <= full) {
            item.classList.add("rating__item--active");
            item.style.backgroundImage = `url("data:image/svg+xml;base64,${btoa(generateStar2(true, color))}")`;
          } else if (starIndex === full + 1 && partial > 0) {
            const span = document.createElement("span");
            span.style.width = `${partial * 100}%`;
            span.style.backgroundImage = `url("data:image/svg+xml;base64,${btoa(generateStar2(true, color))}")`;
            item.appendChild(span);
          }
        });
      });
    }, generateStar2 = function(active, color) {
      const path = active ? `M10.0923 14.8835L16.0647 18.4882L14.4798 11.6943L19.7563 7.12325L12.8079 6.53374L10.0923 0.126465L7.37668 6.53374L0.428223 7.12325L5.7048 11.6943L4.11989 18.4882L10.0923 14.8835Z` : `M19.8157 7.12325L12.8672 6.52407L10.1516 0.126465L7.43601 6.53374L0.487549 7.12325L5.76413 11.6943L4.17922 18.4882L10.1516 14.8835L16.124 18.4882L14.5391 11.6943L19.8157 7.12325Z`;
...
<svg width="20" height="19" viewBox="0 0 20 19" xmlns="http://www.w3.org/2000/svg">
    ...
    var colorizeSlides = colorizeSlides2, generateStar = generateStar2;
    new Swiper(".coments__slider", {
